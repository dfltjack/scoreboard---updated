...
var fight_rules = {
  ...
  osaekomi_wazari_time: 15 * 1000,
  osaekomi_yuko_time: 10 * 1000,
  ...
  count_shidos_towards_ippon: 3,
  count_shidos_towards_wazaris: 2,
  count_shidos_towards_yukos: 1,
  // sound
...
function get_initial_fight_state() {
  ...
  console.assert(
    fight_rules.total_fight_time_ms % master_timer_ms == 0,
    "Total fight time invalid"
  );
  if (fight_rules.osaekomi_warn_unassigned != null) {
    console.assert(
      fight_rules.osaekomi_warn_unassigned % master_timer_ms == 0,
      "Osaekomi warn time invalid"
    );
  }
  if (fight_rules.osaekomi_wazari_time != null) {
    console.assert(
      fight_rules.osaekomi_wazari_time % master_timer_ms == 0,
      "Osaekomi wazari time invalid"
    );
  }
  if (fight_rules.osaekomi_yuko_time != null) {
    console.assert(
      fight_rules.osaekomi_yuko_time % master_timer_ms == 0,
      "Osaekomi yuko time invalid"
    );
  }
  if (fight_rules.osaekomi_ippon_time != null) {
    console.assert(
      fight_rules.osaekomi_ippon_time % master_timer_ms == 0,
      "Osaekomi ippon time invalid"
    );
  }
  if (fight_rules.osaekomi_max_time != null) {
    console.assert(
      fight_rules.osaekomi_max_time % master_timer_ms == 0,
      "Osaekomi max time invalid"
    );
  }
...
function add_point(fighter, point_name) {
  ...
  convertShidoToIppon(fighter);
  convertWazariToIppon(fighter);
  // convertYukoToWazari(fighter);
  convertYukoToIppon(fighter);

  function convertShidoToIppon(fighter) {
    if (fight_state.points[fighter]["shido"] === 3) {
      fight_state.points[fighter]["shido"] === 3;
      fight_state.points[fighter]["ippon"]++;
    }
  }

  function convertShidoToWazari(fighter) {
  ...
  function convertShidoToYuko(fighter) {
    if (fight_state.points[fighter]["shido"] == 1) {
      fight_state.points[fighter]["shido"] === 1;
      fight_state.points[fighter]["yuko"] += 1;
    }
  }
  ...
  //     if (!fight_state.points[fighter]["yuko_converted"]) {
  //       fight_state.points[fighter]["wazari"]++;
  //       fight_state.points[fighter]["yuko_converted"] = true;
  //     }
  //   } else {
  //     // Reseta a flag se o número de Yukos não está em um ponto de conversão
  //     fight_state.points[fighter]["yuko_converted"] = false;
  //   }
  // }
  ...
  if (ippon_stop || wazari_stop || yuko_stop || shido_stop) {
    ring_bell();
    matte();
  }
  ...
  // Atualiza o display após a alteração dos pontos
  update_display(fighter, point_name);
}
...
function get_n_ippons(fighter) {
  ...
  if (fight_rules.count_wazaris_towards_ippon != null) {
    n_ippons += Math.floor(
      fight_state.points[fighter]["wazari"] /
        fight_rules.count_wazaris_towards_ippon
    );
  }
  return n_ippons;
}
...
const osaekomi_assign_original_width =
  document.getElementById("osaekomi_assign_0").style.width;
const osaekomi_assign_text_original_width = document.getElementById(
  "osaekomi_assign_text"
).style.width;
...
function update_display() {
  ...
  const total_fight_time_reset_el = document.getElementById(
    "total_fight_time_reset_time"
  );
  total_fight_time_reset_el.innerHTML = format_time_minutes(
    fight_rules.total_fight_time_ms
  );
  ...
  central_clock_el.innerHTML = format_time_minutes(
    fight_state.central_clock_ms
  );
  const central_clock_tenths_el = document.getElementById(
    "central_clock_time_tenths"
  );
  central_clock_tenths_el.innerHTML = format_time_tenths(
    fight_state.central_clock_ms
  );

  // central clock buttons: update based on state
  const pause_continue_central = document.getElementById(
    "central_clock_pause_continue"
  );
  const pause_continue_central_img = document.getElementById(
    "central_clock_pause_continue_img"
  );
  ...
  if (
    ...
    if (
      fight_state.osaekomi_running &&
      fight_state.osaekomi_ms % fight_rules.osaekomi_error_sound_frequency_ms ==
        0
    ) {
      if (!is_view) {
  ...
  const start_stop_osae_img = document.getElementById(
    "osaekomi_start_stop_img"
  );
  ...
  const pause_continue_osae = document.getElementById(
    "osaekomi_pause_continue"
  );
  const pause_continue_osae_img = document.getElementById(
    "osaekomi_pause_continue_img"
  );
...
function total_fight_time_reset_change() {
  const minutes_input = document.getElementById(
    "total_fight_time_reset_minutes"
  );
  const minutes = get_number_from_input(minutes_input);

  const seconds_input = document.getElementById(
    "total_fight_time_reset_seconds"
  );
...
function golden_score_time_set_change() {
  const minutes_input = document.getElementById(
    "golden_score_time_set_minutes"
  );
  const minutes = get_number_from_input(minutes_input);

  const seconds_input = document.getElementById(
    "golden_score_time_set_seconds"
  );
  const seconds = get_number_from_input(seconds_input);
...
function master_timer_handler() {
  ...
  const average_call_frequency =
    (now - master_timer_first_tick_ms) / master_timer_n_calls;
  ...
  if (
    delta >= master_timer_ms * 1.1 &&
    delta > master_timer_delta_max_reported &&
    master_timer_n_calls >= 1000
  ) {
    master_timer_delta_max_reported = delta;
    console.warn(
      `Master timer is behind. We would like to run it every ${master_timer_ms}ms, now is ${now}ms and the next tick is only at ${master_timer_next_tick_ms}ms (Delta: ${delta}ms)`
    );
  }
